...
module.exports = {
    getZ(code) {
        ...
        let z = this.catchZ(code);
        if (z.mul) return this.restoreGroup(z);
        let ans = [];
    ...
    catchZGroup(code, groupPreStr) {
        ...
        for (let preStr of groupPreStr) {
            let content = code.slice(code.indexOf(preStr)),
                z = [];
            let key = preStr.slice(9,-2);

            content = content.slice(content.indexOf("(function(z){var a=11;"));
            content = content.slice(0, content.indexOf("})(__WXML_GLOBAL__.ops_cached."+ key.slice(2))) + "})(z);";
...
//           function gz$6d616b652f_1(){
// if( __WXML_GLOBAL__.ops_cached.$6d616b652f_1)return __WXML_GLOBAL__.ops_cached.$6d616b652f_1
// __WXML_GLOBAL__.ops_cached.$6d616b652f_1=[];
        // gz$6d616b652f_1
        let reg = new RegExp(`function gz\\${wxmart}_\\d+\\(\\)`,"g")
        let groupTest = code.match(reg);
        // let groupTest = code.match(/function gz\$gwx(\d*\_\d+)\(\)\{\s*if\( __WXML_GLOBAL__\.ops_cached\.\$gwx\d*\_\d+\)/g);
        if (groupTest !== null) return this.catchZGroup(code, groupTest);
        let z = [],
            vm = new VM({
                sandbox: {
                    z: z,
        ...
        let lastPtr = code.lastIndexOf(`(z);__WXML_GLOBAL__.ops_set.${wxmart}=z;`);
